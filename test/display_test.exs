defmodule BrainwormsTest.DisplayTest do
  ...
  test "correctly replaces sublist" do
    ...
    result = Brainworms.Display.replace_sublist(original, 2, [:a, :b])
    assert result == expected
  end

  test "display initializes gracefully regardless of SPI availability" do
    # Start a display instance
    {:ok, display_pid} = GenServer.start_link(Brainworms.Display, :ok)
  ...
  test "set_activations works in simulation mode" do
    ...
    result = Brainworms.Display.set_activations(nil, :simulation, activations)
    assert result == :ok
  end

  test "demo functions work in simulation mode" do
    # All demo functions should work in simulation mode without crashing
    assert Brainworms.Display.breathe_demo(nil, :simulation) == :ok
    assert Brainworms.Display.layer_demo(nil, :simulation) == :ok
    assert Brainworms.Display.step_demo(nil, :simulation) == :ok
    assert Brainworms.Display.set_one(nil, :simulation, 0, 1.0) == :ok
    assert Brainworms.Display.set_all(nil, :simulation, 0.5) == :ok
  end
end
